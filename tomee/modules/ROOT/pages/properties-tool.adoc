...
The only warning is that any properties of type ``<id>.password` will have their values masked, so make sure you edit them if you reapply them back into conf/openejb.xml or conf/system.properties.
...
In an embedded environment, the properties can be added to the Hashtable passed into the {ee-prefix}.naming.InitialContext when using the LocalInitialContextFactory or also to the System.getProperties() object before OpenEJB is embedded (which will be when the first InitialContext is created).
...
Default\ MDB\ Container.MessageListenerInterface={ee-prefix}.jms.MessageListener
...
Default\ JDBC\ Database.ConnectionInterface={ee-prefix}.sql.DataSource
...
Default\ Unmanaged\ JDBC\ Database.ConnectionInterface={ee-prefix}.sql.DataSource
...
#
Default\ JMS\ Connection\ Factory.ConnectionInterface={ee-prefix}.jms.ConnectionFactory, \
{ee-prefix}.jms.QueueConnectionFactory, {ee-prefix}.jms.TopicConnectionFactory
