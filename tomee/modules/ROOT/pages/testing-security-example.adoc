...
Builds upon the link:injection-of-entitymanager-example.html[Injection of EntityManager Example]  but adds the use of _@RolesAllowed_ and _@PermitAll_ in the @Stateful bean to restrict who can perform create, persist and remove operations on the EntityManager.
...
See the link:security-annotations.html[Security Annotations]  page for a full description of how the security annotations work.

_The source for this example is in the "testing-security" directory located in the link:openejb:download.html[openejb-examples.zip]  available on the http://tomee.apache.org/downloads.html[download page]._

+++<a name="TestingSecurityExample-TheCode">++++++</a>+++

= The Code

Just as with the link:testing-transactions-example.html[Testing Transactions Example]  the magic of this unit test is in the _ManagerBean_ and _EmployeeBean_ @Stateless beans that we've tucked into our TestCase as inner classes.
...
See the link:injection-of-datasource-example.html[Injection of DataSource Example]  for an explanation of how any Resource can be configured via properties in the TestCase itself or via an openejb.xml file._
