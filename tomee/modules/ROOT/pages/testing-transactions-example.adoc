...
If you were to take the link:injection-of-entitymanager-example.html[Injection of EntityManager Example]  which uses an EXTENDED persistence context and switch it to a TRANSACTION persistence context making no other changes, you'd find that the test would fail.
...
Using a link:unit-testing-transactions.html[transaction in your unit test]  can make testing these scenarios possible.
...
See the link:transaction-annotations.html[Transaction Annotations]  page for a full description of the available transaction attributes.
...
See the link:injection-of-datasource-example.html[Injection of DataSource Example]  for an explanation of how any Resource can be configured via properties in the TestCase itself or via an openejb.xml file.
