...
As noted in the xref:injection-of-env-entry-example.adoc[Injection of env-entry Example] , the EJB 3.0 supported env-entry types are fairly limited.
...
The source for this example is the "custom-injection" directory located in the xref:downloads.adoc[openejb-examples.zip] available on the http://tomee.apache.org/downloads.html[download page].
...
In the "custom-injection" directory of the xref:openejb:download.adoc[examples zip], just run:
