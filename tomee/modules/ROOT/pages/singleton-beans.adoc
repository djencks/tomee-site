...
In my opinion, the {ee-prefix}.ejb.Singleton will replace a lot of what people are using @Stateless for today.
...
Here the 'synchronization' keyword is allowed as well as the full {ee-prefix}.util.concurrent set of libraries.
...
In the event it is not possible to acquire the lock a _{ee-prefix}.ejb.ConcurrentAccessException_ or _{ee-prefix}.ejb.ConcurrentAccessTimeoutException_ will be thrown.
