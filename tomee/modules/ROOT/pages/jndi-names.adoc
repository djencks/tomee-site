...
* One or more of your business remote interfaces extend from {ee-prefix}.rmi.Remote
...
=== Business interfaces extending {ee-prefix}.rmi.Remote

Per spec rules many runtime exceptions (container or connection related) are thrown from {ee-prefix}.rmi.Remote proxies as java.rmi.RemoteException which is not a runtime exception and must be throwable via the proxy as a checked exception.
...
This is fine for a bean class as it does not need to throw the RMI required {ee-prefix}.rmi.RemoteException.
However if we create a proxy from these two interfaces it will also wind up with a 'doIt(){}' method that cannot throw {ee-prefix}.rmi.RemoteException.
