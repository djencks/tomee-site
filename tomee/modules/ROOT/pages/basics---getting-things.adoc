...
Generally speaking the only way to get a xref:containers-and-resources.adoc.adoc#_resources[Container-Managed Resource]  is via _dependency injection_ or _lookup_ from within a xref:containers-and-resources.adoc.adoc#_components[Container-Managed Component] .

The _unbreakable rules_.
Read these over and over again when things don't work.

. java:comp/env is the spec defined namespace for lookup of any xref:containers-and-resources.adoc.adoc#_containers[Container-Managed Resource]
...
. only xref:containers-and-resources.adoc.adoc#_components[Container-Managed Components], _not_ their libraries, can xref:declaring-references.adoc[Declare References]  via xml or annotation
. only xref:containers-and-resources.adoc.adoc#_components[Container-Managed Components], _not_ their libraries, can get dependency injection of [Container-Managed Resource] s
. only xref:containers-and-resources.adoc.adoc#_components[Container-Managed Components], _and_ their libraries, may lookup from java:comp/env
...
If a xref:containers-and-resources.adoc.adoc#_components[Container-Managed Component]  wants to lookup the EJB they must [Declare a Reference|Declaring References]  to it via the @EJB annotionation or <ejb-local-ref> or <ejb-ref> in xml.
