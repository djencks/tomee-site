...
Before we start, if you take a peek in the project pom.xml and look for the maven-dependency-plugin usage you will see that we pull in two completely different driver files for this example.

= AlternateDataSourceTest.java

This test utilizes the Arquillian testing framework.
See http://tomee.apache.org/arquillian-available-adapters.html[here] for more details.
